...
export const CatalogShowcasesSection: React.FC<CatalogShowcasesSectionProps> = ({
  ...
  return (
    <section className="py-4 sm:py-8 px-2 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6 sm:space-y-10">
      {/* Page Title & Intro */}
      <div className="bg-base rounded-3xl p-5 sm:p-8 border border-border shadow-xs relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl space-y-2 sm:space-y-3">
          ...
          <h1 className="text-2xl sm:text-4xl font-extrabold text-text-heading tracking-tight font-serif">
        ...
        {CATALOG_SHOWCASES.map((catalog) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-xs sm:text-sm font-bold transition-all flex items-center gap-2 cursor-pointer border whitespace-nowrap ${
                isActive
                  ? 'bg-inverse text-text-on-inverse border-border shadow-xs'
                  : 'bg-base text-text-muted hover:text-text-heading border-border hover:border-brand'
              }`}
            >
      ...
      <div className="bg-gradient-to-br from-stone-900 to-stone-800 text-white rounded-3xl overflow-hidden shadow-md border border-stone-700/60 relative">
        ...
        <div className="relative z-10 p-5 sm:p-10 space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <h2 className="text-2xl sm:text-4xl font-extrabold font-serif text-white pt-1 sm:pt-2">
                {activeCatalog.name}
              </h2>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <a
                ...
                className="px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-base/10 hover:bg-base/20 text-white text-xs font-bold transition-all border border-white/20 flex items-center gap-1.5 backdrop-blur-xs"
              >
                <Globe className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand" />
                <span>Official Site</span>
                <ExternalLink className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white/70" />
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-[#171a21] hover:bg-[#2a475e] text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-xs cursor-pointer border border-white/10"
              >
                <ShoppingBag className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#66c0f4]" />
                <span>Steam Publisher</span>
                <ExternalLink className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white/70" />
              </a>
            </div>
          </div>

          <p className="text-stone-300 text-xs sm:text-base leading-relaxed max-w-3xl">
            {activeCatalog.description}
          </p>

          <div className="bg-stone-900/80 p-3 sm:p-4 rounded-2xl border border-stone-700/80 text-xs sm:text-sm text-stone-200 italic flex items-start gap-2.5 sm:gap-3">
            <span className="text-amber-400 font-serif text-xl sm:text-2xl leading-none">“</span>
            <span>{activeCatalog.curatorQuote}</span>
          </div>

          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-1 sm:pt-2">
            <span className="text-xs text-stone-400 font-semibold mr-1 sm:mr-2">Featured Tags:</span>
            {activeCatalog.featuredTags.map((tag) => (
              <span 
                key={tag}
                className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg bg-stone-800/90 text-stone-300 text-[10px] sm:text-xs font-medium border border-stone-700"
              >
                #{tag}
              </span>
            ))}
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl sm:text-2xl font-bold text-text-heading font-serif">
        ...
        {catalogGames.length === 0 ? (
          <div className="bg-base rounded-2xl p-8 sm:p-12 text-center border border-border space-y-2">
            <p className="text-text-muted text-sm font-medium">No titles matched for this showcase in current filter view.</p>
          </div>
        ) : (
